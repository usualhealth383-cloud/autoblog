    ...
    r = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content":
                ...
                "- 유쾌·친근한 입말체 + 이모지로 생동감. **마지막 줄은 댓글을 부르는 가벼운 질문**으로 "
                "끝내라(예: '여러분은 어떠세요?', '혹시 좋은 팁 있으세요?'). 자연스럽게, 억지 금지.\n"
    ...
    r = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content":
                ...
                "- 새로 쓴 '도입 문단' 1개(2~3문장, 독자 상황·고민에 공감하는 후킹 — 네이버 홈판 느낌).\n"
                "- 본문 섹션 4~5개. 각 섹션 heading(이모지 1개 가능) + paragraphs(각 문단 2~3문장으로 "
                "짧게 → 네이버 모바일에서 스크롤하며 술술 읽히게, 빽빽한 긴 문단 금지).\n"
