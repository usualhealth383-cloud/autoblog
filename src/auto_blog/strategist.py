...
EVERGREEN: list[tuple[str, str, str]] = [
    ...
    ("청약통장", "finance", "🏠 청약통장 아직도 안 만들었다면? 기초 총정리"),
    ("비상금 모으기", "finance", "💸 비상금, 얼마가 적당할까? 현실적으로 모으는 법"),
    ("복리의 마법", "finance", "✨ 복리가 뭐길래? 일찍 시작할수록 좋은 이유"),
    ("환율 기초", "finance", "💱 환율 오르면 내 돈은? 알아두면 쓸모있는 환율 상식"),
    ("신용점수 올리기", "finance", "📈 신용점수 어떻게 올리지? 생활 속 꿀팁 모음"),
    ("분산투자", "finance", "🧺 계란을 한 바구니에? 분산투자 쉽게 이해하기"),
    ("예금 vs 적금", "finance", "🏦 예금이랑 적금, 뭐가 다르지? 상황별 선택법"),
]
...
def _build_candidate(item: trends.TrendItem) -> TopicCandidate | None:
    ...
    if category == "finance":
        ...
        score += 1.5
        reasons.append("연예=고클릭이나 단가 낮음(+1.5)")
