...
# 블로거(메인) 본문 목표 분량 — 약 1500~2000자(주제 맞게 유연).
# 네이버용은 variants.make_naver 가 ~2000자로 따로 재서술, 스레드는 짧게 따로 생성.
...
- 도입부는 독자의 상황·고민에 공감하는 후킹으로 시작하고, 위트·공감·호기심 자극으로 끝까지 읽게 만든다.
- **네이버 홈/모바일에서 스크롤하며 술술 읽히는 '홈판 스타일'**로 써라: 문단은 2~3문장으로 짧게,
  소제목으로 잘게 나눠 한눈에 스캔되게. 핵심은 한 줄로 콕 집어주고, 긴 호흡의 빽빽한 문단은 금지.
  ...
  "sections": [
    {
      ...
      "paragraphs": ["문단", "문단(각 문단 2~3문장으로 짧게 — 모바일 가독성)"],
