#!/usr/bin/env python3
"""약손 앱 빌드 — content/*.json 을 app-shell.html 에 심어 docs/yakson/index.html 로 낸다.

사용:  python3 yakson/tools/build.py
- content/ 의 모든 JSON 을 하나의 DATA 객체로 합친다(파일명 = 키).
- public/ 에 공공데이터(e약은요·낱알식별) 추출본이 있으면 함께 심는다.
- 배포본은 GitHub Pages(docs/yakson/) 로 나간다. manifest·sw 는 같은 폴더에 둔다.
"""
...
pages = ROOT.parent / 'docs' / 'yakson'
pages.mkdir(parents=True, exist_ok=True)
(pages / 'index.html').write_text(out, encoding='utf-8')
# 서비스워커 캐시 이름을 빌드마다 갱신
sw = ROOT / 'sw.js'
if sw.exists():
    s = re.sub(r"const CACHE = '[^']*'", f"const CACHE = 'yakson-{stamp}-{abs(hash(j)) % 10000}'", sw.read_text(encoding='utf-8'))
