...
- **블로거(메인):** ~1500~2000자 본문 + 섹션별 사진 → 자동 발행
...
③본문 생성        writer.py        GPT 초안 → Gemini 교차검수 → 엄격 검증 → 블로거 ~1500~2000자
...
- **채널별 분량:** 블로거 ~1500~2000자(메인) / 네이버 ~2000자(재서술) / 스레드 짧게.
...
- [x] 3. 본문 생성 (writer.py, GPT) — 1500~2000자 실동작. Gemini/Claude 검수는 키 넣으면 자동 활성
